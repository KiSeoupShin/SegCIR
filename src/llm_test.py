...
sys.path.append("/home/work/gisub_conference/LLM_test")
...
from model_llm import LlamaSummarize
from PIL import Image

def prepare_img(img_file, transform):
    return transform(Image.open(img_file))

def extract_features(model, img2text, args):        
    model.eval()
    img2text.eval()
    ...
    for query in query_file.split(","):        
        ...
        mask_img = torch.ones_like(query_img[0, :1, :, :]) 
        
        # h, w = query_img.shape[-2:]
        # mask_img = np.ones((h, w), dtype=np.uint8) * 255
        # mask_img = Image.fromarray(mask_img)
        # mask_img = mask_img = segment_model.mask_transform(mask_img)
        ...
        # 특징만 GPU에 남기고 나머지 메모리 정리
        # query_img_feature는 필요하므로 유지하고 CPU로 이동시키지 않음
        del query_img
        del mask_img
        del img_feature
        ...
        model.cpu()
        img2text.cpu()
        torch.cuda.empty_cache()  # 다시 GPU 캐시 비우기
        
        # 추출된 특징만 GPU에 남김
        query_img_feature = query_img_feature.cuda(args.gpu, non_blocking=True)
    
    return query_img_feature

def process_llm(img_embeddings, prompt):
    model = LlamaSummarize()

    output = model.token_test(prompt, img_embeddings)
...
def main_worker(gpu, ngpus_per_node, log_queue, args):
    ...
    img_embeddings = extract_features(model, img2text, args)
    process_llm(img_embeddings, prompt[0])
...
def main(args):
    ...
    args.log_path = os.path.join(args.logs, args.name, f"out_demo_{args.retrieval_data}.log")
...
if __name__ == "__main__":
    config_path = "./configs/demo_cirr.yml"
